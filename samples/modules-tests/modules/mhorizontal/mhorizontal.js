var mhorizontal = (() => {
  const moduleName = 'mhorizontal'

  // эта часть отвечает за создание и работу порождаемых экземпляров объектов 
  const istances = {}
  const createInstance = (instanceName) => {
    if (istances[instanceName]) throw new Error(`mhorizontal.instance '${instanceName}' already exist!`)
...
// console.log('++++ mhorizontal: ', mhorizontal);